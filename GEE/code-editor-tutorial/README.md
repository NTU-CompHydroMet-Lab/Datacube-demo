...
程式最下方已經附上匯出到 Google Drive 的範例，但預設是註解狀態：
...
## 為什麼目前只有一個 JavaScript 檔？

這個範例流程很短，一個 `.js` 檔就能完整呈現「讀資料、處理、視覺化、可選匯出」。

拆成兩個 `.js` 只有在教學變長時才比較有價值，例如：
...
目前先維持單一完整 script，比較方便複製到 GEE Code Editor，也比較容易維護。
